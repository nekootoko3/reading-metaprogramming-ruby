...
class EvilMailbox
  def initialize(mailer, auth_str = nil)
    mailer.auth(auth_str) if auth_str

    self.define_singleton_method(:send_mail) do |to, body, &block|
      body += auth_str if auth_str
      sended = mailer.send_mail(to, body)
      block.call(sended) if block
      nil
    end

    self.define_singleton_method(:receive_mail) do
      mailer.receive_mail
    end
  end
end
